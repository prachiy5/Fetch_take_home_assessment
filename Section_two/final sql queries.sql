...
-------------two ways of doing question first---------
...
-------------WAY 2 (Question 1) with window function: (faster)------------------------------------------------
...
WITH Yearly_User_Growth AS (
    SELECT 
        YEAR(CREATED_DATE) AS registration_year,  -- Extract year from CREATED_DATE
...
-- 3. At what percent has Fetch grown year over year? (Using Window function: LAG)
...
WITH Yearly_User_Growth AS (
    SELECT 
        YEAR(CREATED_DATE) AS registration_year,  -- Extract year from CREATED_DATE
        COUNT(ID) AS new_users,  -- Count number of new users per year
        LAG(COUNT(ID)) OVER (ORDER BY YEAR(CREATED_DATE)) AS previous_users  -- Get previous year's new user count
    FROM users
    GROUP BY YEAR(CREATED_DATE)
)
...
-- Calculate Year-over-Year (YoY) growth percentage
SELECT 
    registration_year AS current_year, 
    new_users AS current_users,  -- New users in the current year
    previous_users,  -- New users in the previous year
    CAST(ROUND((new_users - previous_users) * 100.0 / previous_users, 2) AS DECIMAL(5,2)) AS YoY_growth_percent
FROM Yearly_User_Growth
WHERE previous_users IS NOT NULL  -- Exclude the first year (no previous data)
